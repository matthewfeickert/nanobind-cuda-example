...
def test_matches_scipy(rng, dtype, n, m, d):
    ...
    got = pairwise_numpy.pairwise_distances(x, y)
...
def test_y_defaults_to_x(rng):
    ...
    got = pairwise_numpy.pairwise_distances(x)
...
def test_feature_mismatch_raises(rng):
    with pytest.raises(ValueError, match="same number of columns"):
        pairwise_numpy.pairwise_distances(rng.normal(size=(3, 2)), rng.normal(size=(3, 5)))
