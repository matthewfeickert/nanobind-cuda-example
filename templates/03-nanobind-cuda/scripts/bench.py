...
def main():
    ...
    print(
        f"{'n':>7} {'d':>4} | {'numpy':>9} {'scipy':>9} {'sklearn':>9} {'cuda':>9} | speedup vs scipy"
    )
    for n, d in [(1_000, 16), (4_000, 16), (8_000, 16), (8_000, 128)]:
        x = rng.normal(size=(n, d)).astype(np.float32)
        t_np = timeit(numpy_broadcast, x) if n <= 4_000 else None
        ...
        print(
            f"{n:>7} {d:>4} | {t_np} {t_sp:>8.3f}s {t_sk:>8.3f}s {t_gpu:>8.3f}s | {t_sp / t_gpu:>6.1f}x"
        )
