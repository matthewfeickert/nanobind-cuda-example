# Stage 4: nanobind, the kernel compiled ahead of time

Stage 3 is a perfectly reasonable place to stop, and so is [stage 2](./stage-jax.md) when the problem fits array operations.
...
Stage 4 is the alternative for a package meant for other people.
It takes the kernel from stage 3 unchanged, wraps it in a nanobind binding, and compiles it with `nvcc` when the package is built rather than when it is used.
...
If none of those matter for your project, stage 3 is the better stage and the extra machinery below is cost without benefit.
...
Stage 4 is walked from the outside in.
