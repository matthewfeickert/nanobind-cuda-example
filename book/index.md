...
It is built four times, in four directories under `templates/`, each one a stage a real project might pass through.

1. [Stage 1](./stage-numpy.md) is NumPy only: a pure Python package and the CPU baseline.
2. [Stage 2](./stage-jax.md) is the same broadcast written in [JAX](https://docs.jax.dev/) and compiled by XLA for the GPU. No kernel is written, and the package is still pure Python.
3. [Stage 3](./stage-cuda-python.md) ships the CUDA kernel as source and compiles it at first use with NVRTC through [cuda-python](https://github.com/NVIDIA/cuda-python). Still a pure Python package.
4. [Stage 4](./stage-nanobind.md), `gpu-pairwise`, compiles the same kernel ahead of time with `nvcc` and binds it with nanobind. It is the alternative to stage 3 for a package shipped to other people, and it gets the most chapters because it has the most parts.
...
If you do have an NVIDIA GPU and [Pixi](https://pixi.prefix.dev/latest/installation/) installed, cloning the repository and running `pixi run demo` inside any of the four template directories reproduces the corresponding output.
...
[Four ways to build the same package](./stages.md) introduces the shared API, compares all four in one benchmark, and says when each is worth choosing.
[Stage 1](./stage-numpy.md), [stage 2](./stage-jax.md), and [stage 3](./stage-cuda-python.md) each get one chapter.
[Stage 4](./stage-nanobind.md) opens with the trade-off against stage 3 and is then walked from the outside in.
